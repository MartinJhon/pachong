...
class BookSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        authors = re.search('作者.*?</span>(.*?)<br>',info,re.S).group(1).replace('\n', '').replace(' ','')
        ...
        item['pages'] = " ".join(re.findall('<span.*?页数:</span>\s*(.*?)<br>',info,re.S))#页数
        item['price'] = " ".join(re.findall('<span.*?定价:</span>.*?([0-9\.]+)元?<br>',info,re.S)) #定价
